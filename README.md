...
```shell
docker run -d --rm --name bitrix-application --env SMTP_CONTAINER=smtp --env SMTP_EMAIL=example@mail.com andriykryvenko/bitrix-app:8.4
```
...
services:
  app:
    ...
    restart: always
    image: andriykryvenko/bitrix-app:8.4
    tty: true
    environment:
      SMTP_CONTAINER: "smtp"
